...
newDF = df.copy()
newDF=newDF.loc[:, newDF.columns != '_id']
newDF = newDF.rename(columns = {'time':'event_time', 'Humidity':'Humidity (g.kg-1)',
        ...
        dbc.CardBody(
            [
                html.H4(Deviceid,style = {"background-color":"rgba(0,0,0,0.0)","border-color":"rgb(0,0,0,0.0)",
                                                    "text-align":"center","color":"#cb4154"}, className="card-title"),
    ...
    dbc.CardBody(
        [
            html.H4(id="load_Noreads",style = {"background-color":"rgba(0,0,0,0.0)","border-color":"rgb(0,0,0,0.0)",
                                                "text-align":"center","color":"#cb4154"}, className="card-title"),
            dcc.Interval(id='interval-read-component',interval=60*1000,n_intervals=0,)
           # html.P("This is some card text", className="card-text"),
        ]
    ),
    #dbc.CardFooter("This is the footer"),
        ...
        dbc.CardBody(
            [
                html.H4(id="load_maxHumidity",style = {"background-color":"rgba(0,0,0,0.0)","border-color":"rgb(0,0,0,0.0)",
                                                    "text-align":"center","color":"#cb4154"}, className="card-title"),
                dcc.Interval(id='interval-mhum-component',interval=60*1000,n_intervals=0,),
                #html.P("This is some card text", className="card-text"),
            ],
        ),
        ...
        dbc.CardBody(
            [
                html.H4(id="load_maxTemperature", style = {"background-color":"rgba(0,0,0,0.0)","border-color":"rgb(0,0,0,0.0)",
                                                    "text-align":"center","color":"#cb4154"},  className="card-title"),
                dcc.Interval(id='interval-maxtemp-component',interval=60*1000,n_intervals=0,),
                #html.P("This is some card text", className="card-text"),
            ]
        ),
        ...
        dbc.CardBody(
            [
                html.H4(id="load_minHumidity",style = {"background-color":"rgba(0,0,0,0.0)","border-color":"rgb(0,0,0,0.0)",
                                                    "text-align":"center","color":"#cb4154"},  className="card-title"),
                dcc.Interval(id='interval-minhum-component',interval=60*1000,n_intervals=0,),
                #html.P("This is some card text", className="card-text"),
            ],
        ),
        dbc.CardFooter("(g.kg-1)",style = {"background-color":"rgba(0,0,0,0.0)","border-color":"rgb(0,0,0,0.0)",
         ...
         dbc.CardBody(
             [
                 html.H4(id="load_minTemperature", style = {"background-color":"rgba(0,0,0,0.0)","border-color":"rgb(0,0,0,0.0)",
                                                     "text-align":"center","color":"#cb4154"}, className="card-title"),
                 dcc.Interval(id='interval-mintemp-component',interval=60*1000,n_intervals=0,),
                 #html.P("This is some card text", className="card-text"),
             ]
         ),
         dbc.CardFooter("(°C)",style = {"background-color":"rgba(0,0,0,0.0)","border-color":"rgb(0,0,0,0.0)",
            ...
            dbc.Card(
                [
                   ...
                   html.P(),
                    ...
                    dcc.Interval(id='interval-avghum-component',interval=60*1000,n_intervals=0,),
...
dbc.Col(dbc.Card([dcc.Graph(id='load-hum-graph',figure=fighum),dcc.Interval(id='interval-humgraph-component',interval=60*1000,n_intervals=0,) ],style = {"background-color":"rgba(0,0,0,0.3)","border-color":"#cb4154",},
        ...
        dbc.Col(
            dbc.Card(
                [
                    ...
                    dcc.Interval(id='interval-avgtemp-component',interval=60*1000,n_intervals=0,),
            ],style = {"background-color":"rgba(0,0,0,0.3)","border-color":"#cb4154",}
    ), xs=12,sm=12,md=12,lg=3,xl=3
    ),
           
           
dbc.Col(dbc.Card([dcc.Graph(id='load-temp-graph',figure=figtemp),dcc.Interval(id='interval-tempgraph-component',interval=60*1000,n_intervals=0,) ],style = {"background-color":"rgba(0,0,0,0.3)","border-color":"#cb4154",}
), xs=12,sm=12,md=12,lg=9,xl=9,
style = {"margin-top":"1%",}),
            ],
            className="mb-4",
...
dash_table.DataTable(
                ...
                page_size=6,
            ),dcc.Interval(id='interval-datatable-component',interval=60*1000,n_intervals=0,)],style = {"background-color":"rgba(0,0,0,0.3)","border-color":"#cb4154",}
...
def update_datatable(n_intervals):
    ...
    # df = df.sort_values(by='time', ascending=False)
    # newDF = df.copy()
    # newDF = newDF.rename(columns = {'_id':'event_id', 'time':'event_time', 'Humidity':'Humidity(g.kg-1)',
    #                       'Temperature':'Temperature(°C)'}, inplace = False)
    # Converting time, humidity and temperature into int
    df['time'] = df['time'].astype(int)   # Transform as numeric
    #df['time'] = pd.to_datetime(df['time'])
    df['Humidity'] = df['Humidity'].astype(str) + '  g.kg-1'
    ...
    newDF = df.copy()
    newDF=newDF.loc[:, newDF.columns != '_id']
    newDF = newDF.rename(columns = {'time':'event_time', 'Humidity':'Humidity',
                          'Temperature':'Temperature'}, inplace = False)
